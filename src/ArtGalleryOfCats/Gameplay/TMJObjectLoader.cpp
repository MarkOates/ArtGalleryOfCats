...
void TMJObjectLoader::load()
{
   if (!((!loaded)))
   {
      ...
      error_message << "[ArtGalleryOfCats::Gameplay::TMJObjectLoader::load]: error: guard \"(!loaded)\" not met.";
      std::cerr << "\033[1;31m" << error_message.str() << " An exception will be thrown to halt the program.\033[0m" << std::endl;
      throw std::runtime_error("[ArtGalleryOfCats::Gameplay::TMJObjectLoader::load]: error: guard \"(!loaded)\" not met");
   }
   int tile_width = 1;
   int tile_height = 1;
